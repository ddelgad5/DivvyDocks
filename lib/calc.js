...
exports.findByZip = (zip) => {
  ...
  rpn(options).then( function(results) {
    ...
    for (const station of results.stationBeanList) { // iterate through the json file
      ...
      if (station.postalCode === zip) {  // Look for stations around IIT
    ...
    listChunks = exports.chunkSnip(listChunks, 10);
    let query = {
      timeStamp: Date(results.executionTime),
...
exports.findByCoord = (coord) => {
  console.log("Starting find by coord");
  let query;
  rpn(options).then( function(results) {
    ...
    listChunks = exports.chunkSort(listChunks, coord);
    listChunks = exports.chunkSnip(listChunks, 10);
    query = {
      timeStamp: Date(results.executionTime),
      listChunks: listChunks
    }
    console.log("API calculations finished", query);
  });
  console.log("Finished findByCoord");
  console.log(query);
  return query;
};
